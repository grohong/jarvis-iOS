...
internal struct Part {
    ...
    internal(set) var type: String = "text/plain"

    /// A dictionary of the headers: Content-Type, Content-Disposition, 
    /// Content-Transfer-Encoding.
    internal(set) var headers: [HeaderType: String] = [HeaderType: String]()

    /// The contents of the part.
    internal(set) var body: ParsedBody = .raw(Data())

    // custom, content id - hmkang
    internal(set) var contentID: String?
