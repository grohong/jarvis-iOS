...
internal class MultiPartBodyParser: BodyParserProtocol {
    ...
    init(boundary: String) {
        guard let boundaryData = String("--" + boundary).data(using: .utf8),
            let endBoundaryData: Data = "--".data(using: .utf8),
            let newLineData: Data = "\r\n".data(using: .utf8),
            let endHeaderData: Data = "\r\n\r\n".data(using: .utf8)
            else {
//                LogError("Error converting strings to data for multipart parsing")
                exit(1)
        }
    ...
    func parse(_ data: Data) -> ParsedBody? {
        ...
        var endBoundaryEncountered: Bool = false
    ...
    private func handleHeaderLine(_ line: String, part: inout Part) {
        if let labelRange = getLabelRange(of: "content-type:", in: line) {
            part.type = line.stringByIndex(from: line.index(after: labelRange.upperBound))
        ...
        if let labelRange = getLabelRange(of: "content-disposition:", in: line) {
            ...
            if let nameRange = line.range(of: "name=", options: caseInsensitiveSearch, range: labelRange.upperBound..<line.endIndex) {
                ...
                part.name = line.stringByIndex(from: valueStartIndex, to: (valueEndIndex?.lowerBound ?? line.endIndex))
            }
            if let filenameRange = line.range(of: "filename=", options: caseInsensitiveSearch, range: labelRange.upperBound..<line.endIndex) {
                ...
                let valueEndIndex = line.range(of: "\"", range: valueStartIndex..<line.endIndex)
                part.filename = line.stringByIndex(from: valueStartIndex, to: (valueEndIndex?.lowerBound ?? line.endIndex))
            }
        ...
        if let labelRange = getLabelRange(of: "content-id: ", in: line) {
            part.contentID = line.stringByIndex(from: labelRange.upperBound)
            return
        }
    }
